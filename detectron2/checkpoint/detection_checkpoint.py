...
class DetectionCheckpointer(Checkpointer):
    ...
    def _load_file(self, filename):
        ...
        elif os.path.split(filename)[-1].startswith('swin'):
            loaded = super()._load_file(filename)
            if 'model' not in loaded:
                loaded = {'model': loaded}
            loaded["__author__"] = "swin"
    ...
    def _load_model(self, checkpoint):
        if checkpoint.get("matching_heuristics", False):
            ...
            if checkpoint.get("__author__", None) == "Caffe2":
                conversion = 'c2_conversion'
            elif checkpoint.get("__author__", None) == "swin":
                conversion = 'swin'
            else:
                conversion = ''
