...
def find_top_rpn_proposals(
    ...
    for level_id, proposals_i, logits_i in zip(
        ...
        Hi_Wi_A = logits_i.shape[1]  # logits_i: (N, Hi*Wi*A)
        num_proposals_i = min(pre_nms_topk, Hi_Wi_A)

        # sort is faster than topk (https://github.com/pytorch/pytorch/issues/22812)
        # topk_scores_i, topk_idx = logits_i.topk(num_proposals_i, dim=1)
        logits_i, idx = logits_i.sort(descending=True, dim=1)
        topk_scores_i = logits_i[batch_idx, :num_proposals_i]  # (N, topk)
        topk_idx = idx[batch_idx, :num_proposals_i]            # (N, topk)

        # each is N x topk
        topk_proposals_i = proposals_i[batch_idx[:, None], topk_idx]  # (N, topk, 4), shape broadcast
