...
class FastRCNNOutputs(object):
    ...
    def losses(self):
        ...
        return {
            ...
            "loss_box_reg": self.bbox_reg_loss_weight * self.smooth_l1_loss(),
        }
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "bbox_reg_loss_weight"  : cfg.MODEL.ROI_BOX_HEAD.BBOX_REG_LOSS_WEIGHT,
    ...
    def losses(self, predictions, proposals):
        ...
        return FastRCNNOutputs(
            self.box2box_transform, scores, proposal_deltas, proposals, self.smooth_l1_beta, self.bbox_reg_loss_weight
        ).losses()
    ...
    def predict_boxes(self, predictions, proposals):
        ...
        if not len(proposals):
            return []
        _, proposal_deltas = predictions   # predictions: scores, proposal_deltas
        num_prop_per_image = [len(p) for p in proposals]  # proposals: list[Tensor]: each element is in an image
        proposal_boxes = [p.proposal_boxes for p in proposals]
        proposal_boxes = proposal_boxes[0].cat(proposal_boxes).tensor
        predict_boxes = self.box2box_transform.apply_deltas(
            proposal_deltas, proposal_boxes
