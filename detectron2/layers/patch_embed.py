...
def coordinate_to_encoding(coord_tensor, num_feats=128,
...
class PatchEmbed_origin(nn.Module):
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, in_channels=3, embed_dim=768, kernel_size=16,
                 stride=16, padding='corner', dilation=1, bias=True,
                 norm_layer=nn.LayerNorm, input_size=None):
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=2, stride=None,
                 padding='corner', dilation=1, bias=False, norm_layer=nn.LayerNorm,
                 ):
        super().__init__()
        ...
        if isinstance(padding, str):
            self.adap_padding = AdaptivePadding(kernel_size=kernel_size, stride=stride,
        ...
        self.norm = norm_layer(sample_dim) if norm_layer else nn.Identity()
    ...
    def forward(self, x, input_size):
        ...
        """
        B, L, C = x.shape
        assert isinstance(input_size, Sequence), f'Expect input_size is ' \
                                                 f'`Sequence` but get {input_size}'
